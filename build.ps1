...
$publicFiles = @('index.html', 'styles.css', 'final.css', 'card-layout.css', 'growth.css', 'app.js', 'growth.js', 'robots.txt', '_redirects')
foreach ($file in $publicFiles) {
  Copy-Item -LiteralPath (Join-Path $projectRoot $file) -Destination (Join-Path $outputDirectory $file) -Force
}
$assets = @('favicon.svg', 'llmold-avatar.png', 'llmold-scene-banner.png', 'llmold-banner.png', 'llmold-social-card-v2.png', 'llmold-x-card-v3.jpg')
