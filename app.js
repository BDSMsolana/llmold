(() => {
  ...
  function displayReport(value) { report = { ...value }; incidentId.textContent = report.id; title.textContent = report.nutrient; body.textContent = report.outcome; shareButton.href = `https://x.com/intent/tweet?text=${encodeURIComponent(reportText())}`; incident.hidden = false; document.dispatchEvent(new CustomEvent('llmold:display', { detail: { ...report } })); }
